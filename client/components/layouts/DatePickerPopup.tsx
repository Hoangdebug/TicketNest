    ...
    return (
        <div className="components__event-popup">
            ...
            <div className="components__event-popup-filters d-flex justify-content-between gap-2 my-3">
                <button className={`filter-button ${activeFilter === 'all' ? 'active' : ''}`} onClick={() => handleFilter('all')}>
                    All dates
                </button>
                <button className={`filter-button ${activeFilter === 'today' ? 'active' : ''}`} onClick={() => handleFilter('today')}>
                    Today
                </button>
                <button className={`filter-button ${activeFilter === 'tomorrow' ? 'active' : ''}`} onClick={() => handleFilter('tomorrow')}>
                    Tomorrow
                </button>
                <button className={`filter-button ${activeFilter === 'weekend' ? 'active' : ''}`} onClick={() => handleFilter('weekend')}>
                    This weekend
                </button>
                <button className={`filter-button ${activeFilter === 'month' ? 'active' : ''}`} onClick={() => handleFilter('month')}>
                    This month
                </button>
            </div>
